...
public class AutorRestDaoImpl extends BaseRestDaoImpl<AutorDTO, Long> implements AutorRestDao{
	...
	public AutorDTO clonar(Long idAutor) throws Exception {
	    try {
	        
	        String uri = baseUrl + "/clonar/{id}";
	        ResponseEntity<AutorDTO> response = restTemplate.getForEntity(uri, AutorDTO.class, idAutor);
